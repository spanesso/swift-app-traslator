...
- **A change of speaker after a SHORT utterance is also a restart (field, 2026-09-15):** "when the other person starts talking, their first words are lost". With a short previous utterance nothing collapses, and two paths skipped the new speaker's first words: a 1–2-word anchor ("there") matching inside the new sentence, and reading the new text by POSITION in the old utterance. Now anchors shorter than `TranscriptWindow.trustedAnchorWords` (3) and the positional fallback are believed only if the words just before the committed boundary are mostly the same words (`boundaryOverlap` + `isSameUtterance`: last 8 words as a multiset, ≥3 compared, ≥2/3 shared). NOT position by position: the recogniser inserts and splits words while revising, and a positional comparison mistook those revisions for new speakers and emitted whole utterances twice (second field log, `ASR_RESTART_DETECTED incomingWords=N committedWords=N`). Otherwise `startsNewUtterance` treats it as a restart (flush + reset). Overlap trimming needs ≥2 words — one shared word ("so", "yes") is usually the next speaker's. When in doubt the rule prefers showing a few words twice over losing them. Tests: `SegmenterWordLossTests` "Change of speaker".
